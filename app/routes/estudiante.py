...
def add_student():
    if request.method == "POST":
        ...
        estudiante_cedula = Estudiante.query.filter_by(
            cedula=form_new_student["cedula"]
        ).first()
        estudiante_email = Estudiante.query.filter_by(
        ...
        if (estudiante_cedula is None) and (estudiante_email is None):
            ...
            try:
                flash(
                    f"El estudiante ya se encuentra en la base de datos, por favor relice una búsqueda por el número de cédula: {estudiante_cedula.cedula}",
                    "error",
                )
            except Exception:
                flash(
                    f"El estudiante ya se encuentra en la base de datos, por favor relice una búsqueda por el número de cédula: {estudiante_email.cedula}",
                    "error",
                )
            finally:
                return redirect(url_for("index"))
